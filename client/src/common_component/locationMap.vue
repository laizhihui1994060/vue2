    ...
    <div id="poinMap" :style="{width:width + 'px',height:  parseFloat(height) + 'px' }">
        ...
        <div id="toolTips">
            ...
            <div class="checkboxs" v-if="showInfoDistribution">
                ...
                <div v-if="serverURL(['UnderSingle','showAllPlatformCars'])">
                <!-- getAllPlatformCars -->
                    <checkbox 
                        name='all-platform' 
                        @updata='updataDispatchStatus' 
        ...
        <div v-if="(preMileageGoto!=''&&preMileageGoto!=undefined)||(preMileageArrive!=''&&preMileageArrive!=undefined)" class="fix">
            <button class="default-btn btn" id="start" @click="begin">开始动画</button>
            <button class="default-btn btn" @click="stop">结束动画</button>
        </div>
        <div class="location—wrap">
            <div class="showInfo" v-if="(preMileageGoto!=''&&preMileageGoto!=undefined)||(preMileageArrive!=''&&preMileageArrive!=undefined)">
            ...
            <div class="showInfo bottomDiv" v-if="(mileageGoto!=''&&mileageGoto!=undefined)||(mileageArrive!=''&&mileageArrive!=undefined)">
    ...
    export default {
        ...
        mounted(){
            ...
            that.$nextTick( ()=> {
                ...
                if(this.serverInput){
                    setTimeout(()=> {
                        _Autocomplete(this.dom + "tipinpu");
                    }, 300);
                }
            });
        ...
        data(){
            return {
                ...
                checkboxDisabled:global.chekcedisConnect
            }
        },
        created(){
           

            bus.$on('isConnect',(bool)=>{
                if(bool){
                    if(!this.orgId){
                        setTimeout(()=>{
                            if(!global.ourOrgId){
                                setTimeout(()=>{
                                    this.orgId = global.ourOrgId;
                                    if(!global.ourOrgId){

                                    }else{
                                        this.orgId = global.ourOrgId;
                                        global.chekcedisConnect = false;
        ...
        methods:{
                                ...
                                map_data = content.towTruckDistribution ? content.towTruckDistribution.map((item,index)=>{
                                    ...
                                    return {
                                        ...
                                        str:`<div>车辆类型：${item.category ? item.category : ''}</div>
                                                    ...
                                                    <div>车辆状态：${status}</div>`,
                                        content:`<div class='amap-icon carStatus  ${icon}' style='position: absolute;top:12px;left:-5px'></div>`,
                                        label:item.name
                                    }
                                    ...
                                    if(res.status == 200){
                                        ...
                                        map_data = content.employeeDistribution ? content.employeeDistribution.map((item,index)=>{
                                            ...
                                            return{
                                                ...
                                                content:"<div class='amap-icon carStatus staff' style='position: absolute;top:12px;left:-5px'></div>",
                                                label:item.name
                                            };
                            ...
                            case 'serviceProvider':
                                map_data = content.parkingLotDistribution ? content.parkingLotDistribution.map((item,index)=>{
                                    ...
                                    return {
                                        ...
                                        content:"<div class='amap-icon carStatus  parking' style='position: absolute;top:12px;left:-5px'></div>",
                                        label:item.name2
                                    }
                            ...
                            case 'order':
                                map_data = content.orderIncidentDistribution ? content.orderIncidentDistribution.map((item,index)=>{
                                    ...
                                    return {
                                        ...
                                        content:"<div class='amap-icon carStatus  destinaton' style='position: absolute;top:12px;left:-5px'></div>",
                                        label:item.name
                                    }
            ...
            getAllPlatformCars(name, data){
                
                if(data){
                    this._getAllPlatformCars(name, data );
                }else{
                    _getParking(name, []);
                }
                
            },
            _getAllPlatformCars(name, data, isLunxun){
                send({
                    url: 'order:/towtruckmatching/list/queryallvehicle',
                    type: 'get',
                    isLoading:true,
                }, function(err, res){
                    let content = res.content[0];
                    console.log(res);
                    const map_data_kk = ( data && content.towTruckDistribution ) ? content.towTruckDistribution.map( (item,index)=>{
                        let status = item.status == 'FREE' ? '空闲' : '忙';
                        let icon;
                        // if(item.status){
                        //     icon = item.status.toLowerCase() + 1;
                        // }else{
                        //     status = '空闲';
                        //     icon = 'free' + 1; 
                        // }  
                        ...
                        return {
                            ...
                            str:`<div>当前速度：<span style='color:#F9AC66'>${item.speed ? item.speed : 0}</span> km/h</div>
                                        <div>车牌号码：${item.name ? item.name : ''}</div>`,
                            content:`<div class='amap-icon carStatus  ${icon}' style='position: absolute;opacity: 1;left:-5px;transform: rotate(${item.direction}deg)'></div>`,
                        }
                    }) : [];

                    const map_data_erp = ( data && content.partnerTowTruckDistribution ) ? content.partnerTowTruckDistribution.map( (item,index)=>{
                        let status = item.status == 'FREE' ? '空闲' : '忙';
                        let icon;
                        // if(item.status){
                        //     icon = item.status.toLowerCase() + 1;
                    ...
                    const map_data_allPlatformCars = map_data_kk.concat(map_data_erp);
                    _getParking(name, map_data_allPlatformCars,isLunxun);
                })

            },
            updataDispatchStatus(name,status){
                this.$emit("dispatchTcp",name,status);
            },
            serverURL(url){
