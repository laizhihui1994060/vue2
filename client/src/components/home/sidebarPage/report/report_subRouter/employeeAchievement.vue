...
export default {
    ...
    created(){
        const self = this;
        // var __defStart = 
        var __defEnd = util.getRange("MONTH");

        if(global.tenantId){
            this.iframeSrc = config.globalIframeSrc +  "/report/export?param=%7B%22formatType%22%3A%22HTML%22%2C%22fileName%22%3A%22123%22%2C%22startDate%22%3A%22" + __defEnd.start +"%22%2C%22endDate%22%3A%22"+ __defEnd.end +"%22%2C%22reportType%22%3A%22" + "EMPLOYEE_OUTPUT_VALUE_REPORT" + "%22%2C%22tenantId%22%3A%22" + global.tenantId +"%22%2C%22orgId%22%3A%22" + global.ourOrgId + "%22%7D";

        }else{
            setTimeout(function() {
                
                self.iframeSrc = config.globalIframeSrc + "/report/export?param=%7B%22formatType%22%3A%22HTML%22%2C%22fileName%22%3A%22123%22%2C%22startDate%22%3A%22"+__defEnd.start+"%22%2C%22endDate%22%3A%22"+__defEnd.end+"%22%2C%22reportType%22%3A%22" + "EMPLOYEE_OUTPUT_VALUE_REPORT" + "%22%2C%22tenantId%22%3A%22" + global.tenantId +"%22%2C%22orgId%22%3A%22" + global.ourOrgId + "%22%7D";
    ...
    methods:{
        ...
        setIframeCSS(){
            // const self = this;
            var ele = $('#employeeAchievement').css('height');
            console.log("ELE",ele);
        },
        ...
        JSONToURI(obj){
            ...
            let param, url = config.globalIframeSrc + "/report/export";
            if(obj){
        ...
        setModalConfig(){
            ...
            this.iframeFileName = "人员产值" + __starName + "-" + __endName
