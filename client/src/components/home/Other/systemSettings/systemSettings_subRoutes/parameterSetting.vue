<template>
    <div class="parameter_setting">
                    ...
                    <div class='rightContent'>
                        <template v-if='item.value.values'>
                            <template v-for='obj in item.value.values'> 
                                <checkbox :key='obj.desc' direction='left'  labelSize='12px' :text='obj.desc'  disabled='disabled' :checked='obj.isChoose' top='2px' ></checkbox>
                            </template>
                        </template>
                        <template v-if='item.value.available_columns'>
                            <template  v-for='obj in item.value.available_columns'>
                                <checkbox :key='obj.desc' direction='left'  labelSize='12px' :text='obj.desc'  disabled='disabled'  display='none' :checked='obj.isChoose'></checkbox>
                            </template>
                        </template>
        ...
        <div class='data_dictionary'>
            ...
            <div class='details'>
                <div v-for='item in data_dictionary' :key='item.pathDesc'>
                    <div class="tag">{{item.pathDesc}}</div>
                    <div class='rightContent'>
                        <template v-if='item.value.values[0]'>
                            <template  v-for='obj in item.value.values'>
                                <checkbox :key='obj.desc' direction='left' top='2px'  labelSize='12px' :text='obj.desc'  disabled='disabled' :checked='obj.isChoose' ></checkbox>
                            </template>
                        </template>
        ...
        <div class='system_default_parameter_settings'>
            ...
            <div class="details">
                <div v-for='item in default_parameter_settings' :key='item.pathDesc'>
                    <div class="tag">{{item.pathDesc}}</div>
                    <div class='rightContent'>
                        ...
                        <template v-else>
                            <template v-if='item.value.values[0].code' v-for='obj in item.value.values'>
                                <radio :key='obj.desc' :text='obj.desc' disabled='disabled' top='1px'  :checked='obj.isChoose'  ></radio>
                            </template>
                        </template>
                    </div>
                </div>
        ...
        <div class='business_logic_parameter'>
            ...
            <div class="details"> 
                <div v-for='item in business_logic_parameter_setting' :key='item.pathDesc'>
                    <div class="tag">{{item.pathDesc}}</div>
                    <div class='rightContent'>
                        <template v-if='item.value.type == "radio_options"'>
                            <!-- radio -->
                            ...
                            <template v-if='!item.value.values[0].code' v-for='value in item.value.values'>
                                <radio :key='value' :text='value' disabled='disabled' top='1px'  :checked='obj.isChoose'  ></radio>
                            </template>
                        </template>
                        ...
                        <template v-else-if='item.value.type == "text" '>
                            <template  v-for='obj in item.value.values'>
                                <span :key='obj.code'>{{obj.code}} {{obj.desc}}</span>
                            </template>
<!--                             <template v-if='!item.value.values[0].code' v-for='value in item.value.values'>
                        ...
                        <template v-else>
                            ...
                            <template  v-if='item.path == "/order/driver_report" ' v-for='obj in item.value.values'>
                                <div style="width:100%;" :key='obj.desc'>
                            ...
                            <template v-if='!item.value.values[0].code' v-for='value in item.value.values'>
                                <checkbox :key='value' direction='left' top='2px' labelSize='12px' :text='value' disabled='disabled' :checked='obj.isChoose' ></checkbox>
                            </template>
                        </template>
        ...
        <div class='expense_type'>
            ...
            <div class="details" style='padding:10px 100px;'>
                <table>
                    ...
                    <tbody>
                        <tr v-for='item in expense_type' :key='item.feeTypeCfgId'>
                            <td>{{item.feeName}}</td>
    ...
    export default {
        ...
        methods: {
            updata(){
                ...
                // 技师核对被救车辆信息必填项目
                _this.mainOrgSystemSend('/order/confirm_vehicle_info', function(content){
                    _this.business_logic_parameter_setting.confirm_vehicle_info = content;
                });
                    ...
                    orgSend(path, function(err, res){
                        ...
                        if(res.content.length != 0 ){
                            ...
                            if( path == "/order/message"){
                                ...
                                // input (text)
                                if(!res.content[0].orgParam.value.values || !res.content[0].orgParam.value.values.length){
