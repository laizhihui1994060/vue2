<template>
    <div id='account_modal' class="modal fade" data-backdrop="static">
...
import "../../../../../../static/js/md5.js";
    export default {
        ...
        methods:{
            ...
            setAccount(){
                if(getData('#account_modal')){
                    const _this = this;

                    if(!this.res.password){
                        alert('请输入密码');
                        $('#account_modal input[name="password"]').focus();
                        $('#account_modal input[name="password"]').select();
                        return;
                    }
                    ...
                    if(!/^1(3|4|5|7|8)\d{9}$/.test(data.loginId)){
